...
const getFoodInfo = () => {
  axios.get('FoodController/listFoodByBusinessId', {params:{
    ...
    foodArr.value = response.data.map(food => ({ ...food, quantity: 0 }));
...
const listCart = () => {
  axios.get('CartController/listCart', {params:{
    businessId: businessId,
...
onMounted(() => {
  ...
  // 根据businessId查询商家信息
  axios.post('BusinessController/getBusinessById', qs.stringify({ businessId }))
      .then(response => {
        business.value = response.data;
      })
      .catch(error => {
  ...
  axios.post('FoodController/listFoodByBusinessId', qs.stringify({ businessId }))
      .then(response => {
...
const listCart = () => {
  axios.post('CartController/listCart', qs.stringify({ businessId, userId: user.value.userId }))
      .then(response => {
        const cartArr = response.data;
        // 遍历所有食品列表
